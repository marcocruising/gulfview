...
(
    ...
    'CEPII BACI reconciled bilateral flows at HS6: trade value in USD thousands and quantity in metric tonnes. Default loader filter is V1 HS chapters; optional --exporter-full-hs (repeat per country) / --importer-full-hs add all-HS6 rows for those legs (Exporter & partners and Group dependencies).',
...
(
    ...
    'Merged blocks from Minerals Yearbook Table 2: operating companies, locations, annual capacity; Do. resolved as ditto to previous commodity cell; facility_path includes colon-based hierarchy. excel_row_start/end trace source rows. Optional geocode_lat/lon from scripts/geocode_usgs_facilities.py (Nominatim). Upsert record_fingerprint.',
